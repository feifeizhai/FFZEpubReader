...
@implementation UIImage (JingGeImage)

+ (UIImage *)imagesNamedFromCustomBundle:(NSString *)imgName
{
    NSString *bundlePath = [[NSBundle mainBundle].resourcePath stringByAppendingPathComponent:@"JingGeBundle.bundle"];
   
    NSString *img_path = [[bundlePath stringByAppendingPathComponent:@"pic"] stringByAppendingPathComponent:imgName];
    NSLog(@"%@", img_path);
...
- (UIImage *)toScaleSize:(CGSize)scaleSize
{
    ...
    if(self.size.width>self.size.height)
    {
        if (self.size.width>960) {
            bl=960/self.size.width;
            UIGraphicsBeginImageContext(CGSizeMake(960,self.size.height*bl-1));
            [self drawInRect:CGRectMake(0, 0, 960, self.size.height *bl)];
        }else{
        ...
        if (self.size.width>640) {
            bl=640/self.size.width;
            UIGraphicsBeginImageContext(CGSizeMake(640,self.size.height*bl-1));
            [self drawInRect:CGRectMake(0, 0, 640, self.size.height *bl)];
        }else{
            UIGraphicsBeginImageContext(scaleSize);
            [self drawInRect:CGRectMake(0, 0, scaleSize.width, scaleSize.height)];
        }
