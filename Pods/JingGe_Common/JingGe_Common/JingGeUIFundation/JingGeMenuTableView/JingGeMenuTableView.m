...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        _tableView.backgroundColor  = UIColorFromHex(0xf3f3f3);
...
- (JingGeMenuTableViewCell *)dequeueReusableCellWithIdentifier:(NSString *)identifier forIndexPath:(NSIndexPath *)indexPath{
...
- (void)deleteRowsAtIndexPaths:(NSArray<NSIndexPath *> *)indexPaths withRowAnimation:(JingGeMenuTableViewRowAnimation)animation{
...
- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section
{
    if (self.datasource && [self.datasource respondsToSelector:@selector(numberOfSectionsInTableView:)])
    {
        return [self invoke_heightForFooterInSection:section];
...
{
    UIView* view = [self invoke_viewForFooterInSection:section];
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath*)indexPath {
    ...
    if ([self.delegate respondsToSelector:@selector(mTableView:willDisplayCell:forRowAtIndexPath:)]) {
        [self.delegate mTableView:self willDisplayCell:cell forRowAtIndexPath:indexPath];
    }
}
...
- (void)tableView:(UITableView *)tableView didDeselectRowAtIndexPath:(NSIndexPath *)indexPath {
    [self invoke_didDeselectRowAtIndexPath:indexPath];
}
...
- (void)tapHeader:(UITapGestureRecognizer *) tap {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(mTableView:didSelectSectionAtSection:)]) {
        [self.delegate mTableView:self didSelectSectionAtSection:tap.view];
    }
...
- (void)invode_commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath{
    if (self.datasource&&[self.datasource respondsToSelector:@selector(mTableView:commitEditingStyle:forRowAtIndexPath:)]) {
        JingGeMenuTableViewCellEditingStyle style = (JingGeMenuTableViewCellEditingStyle)editingStyle;
...
- (CGFloat)invoke_heightForFooterInSection:(NSInteger)section{
    if (self.delegate&&[self.delegate respondsToSelector:@selector(mTableView:heightForFooterInSection:)]) {
        return [self.delegate mTableView:self heightForFooterInSection:section];
    }
    return 0;
}
...
- (UIView*)invoke_viewForFooterInSection:(NSInteger)section;{
    if (self.delegate&&[self.delegate respondsToSelector:@selector(mTableView:viewForFooterInSection:)]) {
        return [self.delegate mTableView:self viewForFooterInSection:section];
    }
    return nil;
}
...
- (void)invoke_didDeselectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (self.delegate&&[self.delegate respondsToSelector:@selector(mTableView:didDeselectRowAtIndexPath:)]) {
        [self.delegate mTableView:self didDeselectRowAtIndexPath:indexPath];
    }
}
...
- (void)autoOpenWithSection:(NSInteger)section
{
    ...
    [_multopenSectionArray addObject:[NSNumber numberWithInteger:section]];
    NSArray *insertArray = [self buildInsertRowWithSection:section];
    [_tableView insertRowsAtIndexPaths:insertArray withRowAnimation:UITableViewRowAnimationNone];
    //    [UIView animateWithDuration:0.3 animations:^{
    //        //imageView.transform = CGAffineTransformMakeRotation(0);
    //    }];
}
